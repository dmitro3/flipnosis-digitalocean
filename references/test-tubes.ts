...
<body>
  ...
  <script type="module">
    ...
    const bloomPass = new UnrealBloomPass(
      ...
      0.7,  // Strength - slightly reduced for crispness
      0.2,  // Radius - VERY TIGHT to prevent blur/squareness
      0.3   // Threshold - LOW to catch all pearl emission
    );
    ...
    function showFlipReward(tubeIndex) {
      const tube = tubes[tubeIndex];
      const player = players[tubeIndex];
    ...
    function showGameOverScreen(winnerIndex, winnerName) {
      ...
      contentDiv.style.cssText = `
        ...
        border: 4px solid ${winnerIndex >= 0 ? '#00ff00' : '#ff0000'};
        border-radius: 25px;
        padding: 50px;
        text-align: center;
        box-shadow: 0 0 50px ${winnerIndex >= 0 ? 'rgba(0, 255, 0, 0.5)' : 'rgba(255, 0, 0, 0.5)'};
        max-width: 600px;
        width: 90%;
      `;
      ...
      if (winnerIndex >= 0) {
        contentDiv.innerHTML = `
          ...
          <div style="font-size: 36px; color: #ffffff; margin-bottom: 30px;">
            WINNER: ${winnerName}
          </div>
        ...
        contentDiv.innerHTML = `
          ...
          <div style="font-size: 20px; color: #cccccc; margin-bottom: 30px;">
    ...
    setTimeout(() => {
      ...
      tubes.forEach((tube, tubeIndex) => {
        const player = players[tubeIndex];
        const defaultCoin = coinOptions[0]; // Classic
        const defaultMaterial = coinMaterials[0]; // Graphite
        
        // Force update stored selections
        tube.selectedCoin = defaultCoin;
        tube.selectedMaterial = defaultMaterial;
        
        // Apply material edge color
        const edgeColor = new THREE.Color(defaultMaterial.edgeColor);
        tube.coin.material[0].color.copy(edgeColor);
        tube.coin.material[0].emissive.copy(edgeColor);
        tube.coin.material[0].emissiveIntensity = 0.3;
    ...
    function animate() {
      ...
      tubes.forEach((tube, i) => {
        ...
        // POWER CHARGING SYSTEM - PEARLS GLOW PINK & SPIN WILDLY
        if (tube.isFilling && !tube.isShattered) {
          ...
          // TRANSITION PEARL COLOR from dark to NEON (different color per tube!)
          const darkPearl = new THREE.Color(0x1a1a1a); // Dark grey (neutral start)
          
          // Define neon colors for each tube
          let neonColor;
          if (i === 0) {
            neonColor = new THREE.Color(0x00ff00); // Tube 1: NEON GREEN
          } else if (i === 1) {
            neonColor = new THREE.Color(0x00ddff); // Tube 2: NEON BLUE
          } else if (i === 2) {
            neonColor = new THREE.Color(0xff0088); // Tube 3: NEON PINK
          } else {
            neonColor = new THREE.Color(0xffff00); // Tube 4: NEON YELLOW
          }
        ...
        // PEARLS FLOAT UP & SPIN AROUND - Like a magical snow globe!
        if (tube.foamIntensity > 0 && !tube.isShattered) {
          const spinSpeed = tube.foamIntensity * 0.3; // Rotation speed increases with power
          const spinForce = tube.foamIntensity * 1200; // Orbital force
          const buoyancyForce = tube.foamIntensity * 800; // Upward float force
          ...
          tube.liquidParticles.forEach((particleBody, idx) => {
            ...
            const chaos = Math.sin(frameCount * 0.2 + idx) * 200 * tube.foamIntensity;
            particleBody.force.x += chaos;
            particleBody.force.z += Math.cos(frameCount * 0.2 + idx * 1.3) * 200 * tube.foamIntensity;
          });
        }
