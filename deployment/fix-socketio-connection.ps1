...
foreach ($cmd in $deployCommands) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "🔄 Continuing with next command..." -ForegroundColor Yellow
    }
}
...
try {
    ...
    Write-Host "⚠️ Socket.io endpoint test failed (this might be normal): $($_.Exception.Message)" -ForegroundColor Yellow
}
