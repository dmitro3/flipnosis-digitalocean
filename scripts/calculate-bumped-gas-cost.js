...
async function main() {
  try {
    console.log('🧮 Bumped Gas Cost Calculation...');
    console.log('==================================');
    
    // Bumped parameters
    const depositGasLimit = 15000;
    const gameCreationGasLimit = 25000;
    const maxFeePerGas = BigInt(100000000); // 0.1 gwei (bumped from 0.05)
    const maxPriorityFeePerGas = BigInt(1000000); // 0.001 gwei
    
    console.log('🔧 Bumped Gas Parameters:');
    ...
    console.log(`   Bumped: $${depositGasCostUSD.toFixed(6)}`);
    ...
    if (depositGasCostUSD < 0.01) {
      ...
      console.log('\n⚠️  Gas costs are higher but should be more reliable');
    }
    ...
    console.log('   - Bumped max fee from 0.05 gwei to 0.1 gwei');
    console.log('   - This gives 3x buffer over current network gas price');
    console.log('   - Should eliminate "internal json rpc error"');
    
  } catch (error) {
    console.error('❌ Error calculating bumped gas cost:', error);
  }
}
