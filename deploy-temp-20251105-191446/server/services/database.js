...
class DatabaseService {
    ...
    return new Promise((resolve, reject) => {
      const database = new sqlite3.Database(this.databasePath, (err) => {
        ...
        database.serialize(() => {
          ...
          // Check Battle Royale participants table
          database.get("SELECT name FROM sqlite_master WHERE type='table' AND name='battle_royale_participants'", (err, result) => {
            if (err) {
              console.error('❌ Error checking battle_royale_participants table:', err)
            } else if (result) {
              console.log('✅ Battle Royale participants table exists')
            } else {
              if (allowMigrations) {
                console.log('⚠️ battle_royale_participants table not found - creating it (migrations allowed) ...')
                this.createBattleRoyaleTables(database)
              } else {
                console.error('❌ battle_royale_participants table missing and migrations disabled in production')
              }
            }
          })
  ...
  createBattleRoyaleTables(database) {
    ...
    database.run(`
      CREATE TABLE IF NOT EXISTS battle_royale_games (
        ...
        creator_participates BOOLEAN DEFAULT 0,
        creator_paid BOOLEAN DEFAULT 0,
        nft_claimed BOOLEAN DEFAULT 0,
        game_data TEXT,
  ...
  async createBattleRoyaleGame(gameData) {
    return new Promise((resolve, reject) => {
      ...
      const gameDataJson = gameData.game_data ? JSON.stringify(gameData.game_data) : null
