...
function createApiRoutes(dbService, blockchainService, gameServer) {
    ...
    try {
      ...
      try {
        ...
        if (savedGame) {
          ...
          return res.status(500).json({
            error: 'Game was not found in database after creation',
            details: 'Database verification failed'
          })
        }
      } catch (verifyError) {
        console.error('❌ Error verifying game in database:', verifyError)
  ...
  router.get('/battle-royale', async (req, res) => {
    try {
      console.log('🔍 Battle Royale API called - checking database connection...')
      ...
      console.log('🔍 Fetching Battle Royale games with params:', { status, limit })
      ...
      console.log(`✅ Found ${games.length} Battle Royale games in database`)
      ...
      const gamesWithParticipants = await Promise.all(
        games.map(async (game) => {
          try {
            ...
            console.warn(`⚠️ Error getting participants for game ${game.id}:`, participantError.message)
            // Return game without participants rather than failing entire request
            return {
              ...game,
              current_players: 0,
              participants: []
      ...
      console.log(`✅ Returning ${gamesWithParticipants.length} Battle Royale games with participant data`)
  ...
  router.post('/battle-royale/:gameId/mark-nft-claimed', async (req, res) => {
    try {
      ...
      // Check on-chain state first (authoritative source)
      let onChainState = null
      if (blockchainService.hasOwnerWallet()) {
        try {
          onChainState = await blockchainService.getBattleRoyaleGameState(gameId)
        } catch (error) {
          console.warn('Could not check on-chain state:', error.message)
        }
      }

      // Verify game exists in database or on-chain
      const game = await dbService.getBattleRoyaleGame(gameId)
      if (!game && (!onChainState || !onChainState.success)) {
        return res.status(404).json({ error: 'Game not found in database or on-chain' })
      }

      // Verify winner - check on-chain first if available, otherwise use DB
      if (onChainState && onChainState.success) {
        if (onChainState.winner.toLowerCase() !== winner.toLowerCase()) {
          return res.status(403).json({ error: 'Not the winner of this game (on-chain verification failed)' })
        }
      } else if (game) {
        if (game.winner && game.winner.toLowerCase() !== winner.toLowerCase()) {
      ...
      if (game) {
        ...
        console.log(`✅ NFT claim recorded in database for game ${gameId}`)
      } else {
        console.warn(`⚠️ Game ${gameId} not in database, but NFT claim succeeded on-chain - cannot update DB`)
      }
